...
export const types = {
  ...
  UPDATE_SOURCE_METADATA: `${component}/UPDATE_SOURCE_METADATA`,
...
export default (state: EngineState = initialState, action: any) => {
  switch (action.type) {
    ...
    case types.UPDATE_SOURCE_METADATA:
      return {
        ...state,
...
export const actions = {
  ...
  updateCastSession: (castSession: any) => ({type: types.UPDATE_CAST_SESSION, castSession}),
  ...
  updateSourceMetadata: (sourceMetadata: any) => ({type: types.UPDATE_SOURCE_METADATA, sourceMetadata}),
